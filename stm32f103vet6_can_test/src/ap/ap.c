...
void apMCU1CanTest(void) // 마스터 모드로 작동함.
{
  ...
  canFilter1.FilterIdHigh         = 0x10C << 5;             // Filter ID를 0x10C으로 설정.
                                ...
                                /* Filter 스케일을 16bit로 설정하였기 때문에 Filter ID와 Filter Mask ID의 High와 Low가 의미가 없고 별도의 필터 쌍이 하나더 생기게 됨.
                                                                              아래 설정을 통해 똑같은 기능의 Filter를 하나 더 추가. 사실 별의미 없는 문장. */
  ...
  canFilter1.FilterBank           = 0;                      // 117행 ~127행 까지의 Filter 정보를 Filter Bank 0번에 저장. (Bank 1번은 또 다른 Filter 정보로 설정할 수 있음.)
  ...
  while (1)
  {
    ...
    if (can_rx0_falg == 1) // CAN 수신 인터럽트가 발생하였다면
    {
      ...
      can_rx0_falg = 0; // CAM 수신 인터럽트 플래그 해제.
    }
    ...
    if (uartAvailable(_DEF_UART1) > 0)
    {
      rx_data = uartRead(_DEF_UART1); // UART로 터미널창에서 키입력 받음.
      uartPrintf(_DEF_UART1,"Option : %c\n", rx_data);

      switch(rx_data)
      {
        case 'A':                           // 터미널 창에 'A'를 눌렀을 때
          /* CAN Tx Header 설정 */
